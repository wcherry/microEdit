...
public void lineGetStyle(LineStyleEvent event) {
	...
	while ((token = scanner.nextToken()) != EOF) {
		if (token == OTHER) {
			// do nothing for non-colored tokens
		} else if (token != WHITE) {
			Color color = getColor(token);
			// Only create a style if the token color is different than the widget's default
			// foreground color and the token's style is not bold.  Keywords are bolded.
			if ((!color.equals(defaultFgColor)) || (token == KEY)) {
                ...
                mergeStyles(scanner.getStartOffset() + event.lineOffset, scanner.getLength(), color, fontStyle, styles);
			}
			...
			if (lastStyle.start + lastStyle.length == start) {
				// Have the white space take on the style before it to minimize the number of
				// style ranges created and the number of font style changes during rendering.
    ...
    public void mergeStyles(int start, int len, Color fgColor, int fontStyle, Vector styles){
        StyleRange style = new StyleRange(start, len, fgColor, null);
    ...
    public void removeRange(StyleRange r){
        ranges.remove(r);
    }
